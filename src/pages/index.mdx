...
{/* Design space for surfacing MLLM variations. Each row represents a design dimension that shapes how variations are elicited and presented. The columns list alternative design choices within each dimension. Options marked with backticks indicate the choices implemented in our prototype system: we elicit variants by running multiple *trials* (repeated queries), using different *prompts*, and querying multiple *models* (GPT-4o, Gemini-1.5-Pro, Claude-3.7-Sonnet). We present variations using a *list* format, a *variation-aware description* that integrates agreements and disagreements inline, and a separate *variation summary*. We compare at the granularity of *atomic facts* (individual descriptive claims). We provide three support indicators (*none*, *percentage*, and *source*) to show how many models agree with each claim. We include *model* provenance to identify which models generated each variation. Finally, we present all information as *text* to ensure compatibility with screen readers. */}
...
<Figure>
  <Picture slot="figure" src={preferences} alt="Preferences" invertInDarkMode />
